/**
 * GET /api/scribe/consultations/[id] — enriched consultation + audit trail
 */
...
const log = scribeLogger.child({ component: "API /api/scribe/consultations/[id]" });
...
export async function GET(request, { params }) {
  try {
    ...
    const result = await consultationHistoryService.getDetail(id, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Get consultation detail failed", {
