/**
 * POST /api/scribe/sessions/:id/prescription/review/save
 *
 * Creates an immutable version snapshot of the current draft state.
 * Body (optional): { source: "manual_save", label?: string }
 * Returns: the new version record
 */

import { NextResponse } from "next/server";
import { isScribeError, scribeLogger, toApiError } from "@/features/scribe";
import { resolveScribeContext } from "../../../../../_helpers/context";

const log = scribeLogger.child({
  component: "API /api/scribe/sessions/[id]/prescription/review/save",
});

export async function POST(request, { params }) {
  try {
    const { id } = await params;
    const scribe = await resolveScribeContext(request);
    if (!scribe) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    const { ctx, services } = scribe;

    const body    = await request.json().catch(() => ({}));
    const version = await services.prescriptionReviewService.saveVersion(id, body, ctx);
    return NextResponse.json({ version }, { status: 201 });
  } catch (err) {
    log.error("Save prescription version failed", {
      error: err instanceof Error ? err.message : String(err),
    });
    const apiError = toApiError(err);
    const status   = isScribeError(err) ? err.statusCode : 500;
    return NextResponse.json(apiError, { status });
  }
}
