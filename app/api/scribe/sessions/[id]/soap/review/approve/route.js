...
const log = scribeLogger.child({ component: "API approve SOAP note" });
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await soapReviewService.approve(id, body, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Approve SOAP note failed", { error: err instanceof Error ? err.message : String(err) });
