...
const log = scribeLogger.child({ component: "API reject SOAP note" });
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await soapReviewService.reject(id, body, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Reject SOAP note failed", { error: err instanceof Error ? err.message : String(err) });
