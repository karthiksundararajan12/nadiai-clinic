...
const log = scribeLogger.child({ component: "API save doctor SOAP edits" });
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await soapReviewService.saveDoctorEdits(id, body, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Save doctor SOAP edits failed", { error: err instanceof Error ? err.message : String(err) });
