...
const log = scribeLogger.child({ component: "API SOAP section update" });
...
export async function PATCH(request, { params }) {
  try {
    ...
    const result = await soapReviewService.updateSection(id, body, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Update SOAP section failed", { error: err instanceof Error ? err.message : String(err) });
