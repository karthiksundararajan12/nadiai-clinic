...
const log = scribeLogger.child({ component: "API SOAP version compare" });
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await soapReviewService.compareVersions(id, body, ctx);
    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Compare SOAP versions failed", { error: err instanceof Error ? err.message : String(err) });
