/**
 * POST /api/scribe/sessions/[id]/uploads/retry
 *
 * Issues fresh signed upload URLs for failed or unconfirmed chunks.
 * Body:
...
export async function POST(request, { params }) {
  try {
    ...
    const result = await audioUploadService.retryUpload(id, body, ctx);

    return NextResponse.json(result, { status: 200 });
  } catch (err) {
    log.error("Retry audio upload failed", {
