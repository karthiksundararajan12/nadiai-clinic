...
test("PATIENT_STATUS_CONFIG active maps to confirmed status-pill tokens", () => {
  const active = PATIENT_STATUS_CONFIG.active;
  assert.equal(active.label, "Active");
  assert.equal(active.variant, "success");
  assert.match(active.color, /border-status-confirmed-border/);
  assert.match(active.color, /bg-status-confirmed-bg/);
  assert.match(active.color, /text-status-confirmed/);
});
