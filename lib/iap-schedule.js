 ...
 * can vary by patient history, sex, or state guidance. IAP-ACVIP 2023
 * does not recommend a booster for Typhoid Conjugate Vaccine (TCV).
 *
...
export const IAP_IMMUNIZATION_SCHEDULE = Object.freeze([
  ...
  // MMR - 3 booster (IAP-ACVIP 2023); listed with the 4-6y cohort so the
  // schedule stays chronological. Dose order relative to MMR - 1 / MMR - 2
  // is by name, not array adjacency.
  { vaccineName: "MMR - 3", offsetDays: 1461 },

  // 10 years
  { vaccineName: "Tdap", offsetDays: 3653 },
  { vaccineName: "HPV - 1", offsetDays: 3653 },
  { vaccineName: "HPV - 2", offsetDays: 3835 }, // 6 months after HPV - 1

  // 16-18 years
  { vaccineName: "Td", offsetDays: 5844 },
]);

/**
 * Deduplicated, order-preserving list of every `vaccineName` appearing in
 * IAP_IMMUNIZATION_SCHEDULE (e.g. multi-dose entries like "MMR - 1" /
 * "MMR - 2" / "MMR - 3" stay distinct, since callers select a specific dose
 * by name). Single source of truth for anything that needs "the list of IAP
 * vaccine names" without the offset/scheduling data — e.g. the
 * vaccine-name combobox on the manual /vaccinations/new entry form (see
 * app/(dashboard)/vaccinations/new/page.js). Do not hand-copy this list
 * elsewhere; derive it from here.
 *
 * @type {ReadonlyArray<string>}
 */
export const IAP_VACCINE_NAMES = Object.freeze([
  ...new Set(IAP_IMMUNIZATION_SCHEDULE.map((entry) => entry.vaccineName)),
]);
