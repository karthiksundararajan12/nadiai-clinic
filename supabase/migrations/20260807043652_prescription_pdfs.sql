...
-- Storage path convention: {clinic_id}/{appointment_id}.pdf
-- inside the private `prescriptions` bucket
...
-- Atomic next number for a clinic. Safe under concurrent approvals.
...
-- Persist issued Rx number + storage path on the draft (one draft per session).
...
-- Private bucket for prescription PDFs. Service-role uploads on approval.
